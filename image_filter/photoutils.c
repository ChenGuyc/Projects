...
RGB ApplyFilter(BMP_IMAGE* image, int x, int y)
{
    ...
    for(xf = 0; xf < FilterWidth; ++xf)
    for(yf = 0; yf < FilterHeight; ++yf)
    {
        pixel = GetPixel(image, x + xf - 1, y + yf - 1);
...
{
    LoadImage(argv[1]);
    SetFilterParams(defaultFilter, 3, 3, 1.0, 128.0);
